##### WGCNA for paired normal and tumour samples of TH with clinical information using coding genes
...
##### WGCNA for normal and tumour samples of TH with clinical information using coding genes
...
th_sampledata = aa %>% filter(grepl("Total",Operational_method)) %>% 
...
th_sampledata = th_sampledata[c(aa %>% filter(grepl("Total",Operational_method ) & 
...
if(!dir.exists("th_normal_tumour_samples/")){
  dir.create("th_normal_tumour_samples/")
}

setwd("th_normal_tumour_samples/")
